...
public class TNTCountdownRenderer {
    ...
    public void onWorldRenderLast(RenderWorldLastEvent event) {
        ...
        for (Entity entity : entitiesById) {
            if (entity != null && entity instanceof EntityTNTPrimed && entity
                    .isInRangeToRender3d(renderingVector.getX(), renderingVector.getY(), renderingVector.getZ())) {
    ...
    private void renderFuseCountdown(EntityTNTPrimed passedEntity, float partialTicks, float nameOffset) {
        ...
        if (serverIP.equals("gamster.org") || serverIP.equals("mc.gamster.org")) {
            fuse = passedEntity.fuse - 37;
        } else if (mc.getCurrentServerData() != null) {
            fuse = passedEntity.fuse - 37;
        } else {
            fuse = passedEntity.fuse;
        }
        ...
        double renderPosX = ObfuscationReflectionHelper.getPrivateValue(RenderManager.class, renderManager,
                "field_78725_b"); // renderPosX
        double renderPosY = ObfuscationReflectionHelper.getPrivateValue(RenderManager.class, renderManager,
                "field_78726_c"); // renderPosY
        double renderPosZ = ObfuscationReflectionHelper.getPrivateValue(RenderManager.class, renderManager,
        ...
        GlStateManager.translate((float) (x - renderPosX), (float) (y - renderPosY + passedEntity.height) + nameOffset,
                (float) (z - renderPosZ));
    ...
    private static String ticksToTime(int ticks) {
        if (ticks > 20 * 3600) { // More than 1 hour
            int h = ticks / (20 * 3600); // Convert ticks to hours
            return h + " h";
        } else if (ticks > 20 * 60) { // More than 1 minute
            int m = ticks / (20 * 60); // Convert ticks to minutes
            return m + " m";
        } else { // Less than 1 minute
            int s = ticks / 20; // Convert ticks to seconds
            int ms = (ticks % 20) * 50; // Calculate milliseconds (1 tick = 50 ms)
            return s + "." + ms + " s";
        }
    }
